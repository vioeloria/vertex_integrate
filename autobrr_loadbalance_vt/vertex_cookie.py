...
用法示例：
    ...
    vcm = VertexCookieManager(
        login_url = "http://23.82.99.203:3077",
        username  = "admin",
        password  = "713aa2ac-ddd5-403a-9ddd-4132ce55289a",  # 明文密码，会自动 MD5
    )
    # 或传入已知 MD5 就直接用：
...
class VertexCookieManager:
    ...
    def __init__(
        ...
        logger.debug(f"[VertexCookie] 初始化成功: url={self.login_url}, user={username}")
    ...
    def _save_cookie(self, cookie: str):
        ...
        logger.info("[VertexCookie] Cookie 已更新并缓存")
    ...
    def login(self) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(
                f"[VertexCookie] 登录成功但未提取到 Cookie，HTTP {resp.status_code}"
            )
            return None
...
def md5_password(plaintext: str) -> str:
    ...
    示例：
        >>> md5_password("713aa2ac-ddd5-403a-9ddd-4132ce55289a")
